...
If the `showDataTypes` option is enabled, then type name will be shown near of each primitive value instead of `null`.
...
If the `showObjectSize` option is enabled, then array/object size will be shown near of open line.
...
That option allows to trim long lines after the given length. But a user can click by the string to toggle between
full and trimmed versions of the value.
...
Actions object has following type:
